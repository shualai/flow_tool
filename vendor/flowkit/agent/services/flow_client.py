...
class FlowClient:
    ...
    async def generate_images(self, prompt: str, project_id: str,
        ...
        batch_id = f"{uuid.uuid4()}" if character_media_ids or count > 1 else None
        body = {
            "clientContext": ctx,
            "requests": requests,
        }
    ...
    async def upsample_image(self, media_id: str,
                              project_id: str = "",
                              target_resolution: str = "UPSAMPLE_IMAGE_RESOLUTION_2K",
                              user_paygate_tier: str = "PAYGATE_TIER_ONE") -> dict:
        """Request the same high-resolution image upsample used by Flow downloads."""
        client_context = self._client_context(project_id, user_paygate_tier)
        client_context["recaptchaToken"] = ""
        body = {
        ...
        # The Flow web app calls this endpoint with bearer auth and no API key.
