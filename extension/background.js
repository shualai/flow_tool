...
function connectToBridge() {
  ...
  ws.onmessage = async ({ data }) => {
    try {
      ...
      } else if (msg.method === 'page_api_request') {
        await handlePageApiRequest(msg);
      ...
      if (finalBody.requests && Array.isArray(finalBody.requests)) {
        for (const req of finalBody.requests) {
          ...
          if (req.clientContext) {
            req.clientContext.recaptchaToken = captchaToken;
          }
        }
      }
    }
...
async function handlePageApiRequest(msg) {
  const { id, params } = msg;
  ...
  try {
    ...
    const [{ result }] = await chrome.scripting.executeScript({
      ...
      func: async (request) => {
        ...
        fetchHeaders.authorization = `Bearer ${request.flowKey}`;
